...
public class LeaderBoardSec4B extends RecyclerView.Adapter<LeaderBoardSec4B.ViewHolderUsers> {

    private List<UserModel> mUsers;
    private Context mContext;
    ArrayList<Map.Entry<String, Integer>> list;

    public LeaderBoardSec4B(Context context, List<UserModel> fireUsers) {
        mUsers = fireUsers;
        mContext = context;
    ...
    public ViewHolderUsers onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == 1){
            return new LeaderBoardSec4B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_gold, parent, false));
        }
        if(viewType == 2){
            return new LeaderBoardSec4B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_silver, parent, false));
        }
        if(viewType == 3){
            return new LeaderBoardSec4B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row_bronze, parent, false));
        }
        else {
            return new LeaderBoardSec4B.ViewHolderUsers(mContext, LayoutInflater.from(parent.getContext()).inflate(R.layout.list_leaderboard_row, parent, false));
        }
    }
    ...
    public void onBindViewHolder(ViewHolderUsers holder, int position) {
        ...
        for(int i=0;i<mUsers.size();i++) {
            if (fireUser.getKey().equals(mUsers.get(i).getDisplayName())){
                holder.getUserScoreCount().setText("" + mUsers.get(i).getCountMuhendislik());
    ...
    public void sortingScore() {
        ...
        for(int i=0;i<mUsers.size();i++) {
            ...
            getNameWithScore.put(fireUser.getDisplayName(), fireUser.getMuhendislikB());
            //}
        }
